...
class KeyframeExtractor:
    ...
    def _detect_anomaly_opencv(self, frame: np.ndarray) -> float:
        ...
        if black_ratio > 0.1:  # 超过10%的暗色区域
            anomaly_score += black_ratio * 2.0
        ...
        if yellow_ratio > 0.05:  # 超过5%的黄色区域
            anomaly_score += yellow_ratio * 2.5
        ...
        if red_ratio > 0.15:  # 超过15%的红色区域
            anomaly_score += red_ratio * 1.5

        # 4. 边缘检测（结构异常）
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        edges = cv2.Canny(gray, 50, 150)
        edge_ratio = np.count_nonzero(edges) / total_pixels
        if edge_ratio > 0.2:  # 边缘过多可能表示结构异常
            anomaly_score += 0.3
